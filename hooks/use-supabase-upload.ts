...
interface FileWithPreview extends File {
  ...
  uniqueFileName?: string // Add this to track the unique filename
}
    ...
    const responses = await Promise.all(
      filesToUpload.map(async (file) => {
        ...
        const { error } = await supabase.storage
          .from(bucketName)
          .upload(!!path ? `${path}/${uniqueFileName}` : uniqueFileName, file, {
        ...
        if (error) {
          ...
          return { name: file.name, message: undefined, uniqueFileName }
        }
      })
    )
